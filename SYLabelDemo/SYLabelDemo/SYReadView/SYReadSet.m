...
- (NSAttributedString *)stringWithPosition:(NSInteger)position direction:(SYReadDirection)direction
{
    ...
    if (direction == SYReadNext) {
        ...
        if (position < 0) {
            position = 0;
            return [self stringWithPosition:position direction:SYReadNext];
        }
...
{
    return [self stringWithPosition:self.position direction:SYReadLast];
}

- (NSAttributedString *)nextString
{
   return [self stringWithPosition:(self.position + self.currentWords) direction:SYReadNext];
}
